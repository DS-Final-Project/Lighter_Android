...
import com.example.test_android2.R.layout
...
class cardviewAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            VIEW_TYPE_FRIEND -> {
                val friendView = inflater.inflate(layout.cardview_friend, parent, false)
                FriendViewHolder(friendView)
            }
            VIEW_TYPE_LOVER -> {
                val loverView = inflater.inflate(layout.cardview_lover, parent, false)
                LoverViewHolder(loverView)
            }
            VIEW_TYPE_FAMILY -> {
                val familyView = inflater.inflate(layout.cardview_family, parent, false)
                FamilyViewHolder(familyView)
            }
            VIEW_TYPE_COLLEAGUE -> {
                val colleagueView = inflater.inflate(layout.cardview_colleague, parent, false)
                ColleagueViewHolder(colleagueView)
            }
            else -> {
                val defaultView = inflater.inflate(layout.cardview_none, parent, false)
    ...
    inner class FriendViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        val relation: TextView = view.findViewById(id.tag1)
        val keyword: TextView = view.findViewById(id.tag2)
    }
